...
mod tests {
    ...
    fn test_valid_base58_prefixes() {
        ...
        assert!(is_valid_base58_prefix("JIZZ"));
        assert!(is_valid_base58_prefix("MyWallet"));
    }
    ...
    fn test_expected_iterations_calculation() {
        ...
        assert_eq!(calculate_expected_iterations("ABC"), 97336); // 58^3/2
    }
    ...
    fn test_base58_alphabet_completeness() {
        ...
        for c in valid_chars.chars() {
            assert!(is_valid_base58_prefix(&c.to_string()), "Character '{}' should be valid", c);
        }
    }
